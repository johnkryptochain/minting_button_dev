...
const Home = (props: HomeProps) => {
    ...
    async (commitment: Commitment = 'confirmed') => {
      ...
      if (props.candyMachineId) {
        try {
          ...
          // whitelist mint with address?
          if (process.env.REACT_APP_WHITELIST_ADDRESS === "true") {
            setIsWhitelistUser(false);
            // no whitelist user, no mint
            active = false;
            
            fetch(`${process.env.REACT_APP_BACKEND_URL}/whitelist/addresses`)
            .then(res => res.json())
            .then((res: any) => {
              if (res && res.data) {
                  let walletPubkey: string = anchorWallet.publicKey?.toString();
                  let isWhiteListed: boolean = res.data.includes(walletPubkey);
                  setIsWhitelistUser(isWhiteListed);
                } else {
                  throw "No whitelist address data found.";
                }
              })
              .catch(err => {
                console.log(err);
  ...
  const handlePriceUpdate = (count: number) => {
    let price: number = 0;
    if (isWhitelistUser) {
      price = formatNumber.asNumber(discountPrice) || 0;
    } else if (candyMachine) {
      price = formatNumber.asNumber(candyMachine?.state.price) || 0;
    }

    setTotalPrice(parseFloat(((count * (price + solFeesEstimation))).toFixed(3)));
  }
    ...
    try {
      ...
      if (wallet.connected && candyMachine?.program && wallet.publicKey) {
        ...
        if (mintCount === 1) {
          ...
          mintResult = await mintMultipleToken(
            candyMachine,
            wallet.publicKey,
            mintCount,
            beforeTransactions,
        ...
        if (mintResult && mintResult.length > 0) {
          for (let result of mintResult) {
            if (result.mintTxIds && result.mintTxIds.length > 0) {
              for (let mintTxId of result.mintTxIds) {
        ...
        for (let transactionStatus of allTransactionsResult) {
        ...
        if (totalFailure === 0) {
          if (totalSuccess === mintCount) {
            let remaining = itemsRemaining! - mintCount;
            ...
            <>
                    ...
                    {isActive && endDate && Date.now() < endDate.getTime() ? (
                      ...
                      <>
                        <MintCountdown
                          ...
                          style={{ justifyContent: 'flex-end' }}
                          status={
                            process.env.REACT_APP_WHITELIST_ADDRESS === "true"
                              ? 'WHITELIST ONLY'
              ...
              <MintContainer>
                {candyMachine?.state.isActive &&
                  ...
                  <>
                    {
                      (candyMachine && !candyMachine?.state.gatekeeper) &&
                      <MintCounter mintCount={mintCount} totalPrice={totalPrice} handleMintCount={handleMintCount} />
                    }
                    <MintButton
                      candyMachine={candyMachine}
                      isMinting={isUserMinting}
